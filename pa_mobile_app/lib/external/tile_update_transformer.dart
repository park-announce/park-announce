...
import 'package:pa_mobile_app/external/util.dart';
...
/// update.
typedef TileUpdateTransformer = StreamTransformer<TileUpdateEvent, TileUpdateEvent>;
...
abstract class TileUpdateTransformers {
  ...
  static final ignoreTapEvents = TileUpdateTransformer.fromHandlers(handleData: (event, sink) {
  ...
  static final alwaysLoadAndPrune = TileUpdateTransformer.fromHandlers(handleData: (event, sink) {
    sink.add(event);
  });
  ...
  static bool _triggeredByTap(TileUpdateEvent event) =>
      event.mapEvent is MapEventTap || event.mapEvent is MapEventSecondaryTap || event.mapEvent is MapEventLongPress;
}
