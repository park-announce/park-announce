...
abstract class Crs {
  ...
  LatLng pointToLatLng(Point point, double zoom) => projection.unproject(transformation.untransform(point, scale(zoom).toDouble()));
...
class Proj4Crs extends Crs {
  ...
  /// Converts a map point to the sphere coordinate (at a certain zoom).
  @override
  LatLng pointToLatLng(Point point, double zoom) => projection.unproject(_getTransformationByZoom(zoom).untransform(point, scale(zoom).toDouble()));
...
class _LonLat extends Projection {
  static final Bounds<double> _bounds = Bounds<double>(const Point<double>(-180, -90), const Point<double>(180, 90));
...
class SphericalMercator extends Projection {
  ...
  Point<double> project(LatLng latlng) {
    ...
    return Point(r * latlng.longitude * d, r * math.log((1 + sin) / (1 - sin)) / 2);
  }

  @override
  LatLng unproject(Point point) {
    const d = 180 / math.pi;
    return LatLng(inclusiveLat((2 * math.atan(math.exp(point.y / r)) - (math.pi / 2)) * d), inclusiveLng(point.x * d / r));
  }
}
...
class _Proj4Projection extends Projection {
  ...
  @override
  Point<double> project(LatLng latlng) {
    final point = epsg4326.transform(proj4Projection, proj4.Point(x: latlng.longitude, y: latlng.latitude));
  ...
  @override
  LatLng unproject(Point point) {
    final point2 = proj4Projection.transform(epsg4326, proj4.Point(x: point.x as double, y: point.y as double));
