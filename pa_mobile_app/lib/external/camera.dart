...
class MapCamera {
  ...
  /// the map is rotated.
  Point<int> get pixelOrigin => _pixelOrigin ?? (_pixelOrigin = (project(center, zoom) - size / 2.0).round());
  ...
  static MapCamera? maybeOf(BuildContext context) => FlutterMapInheritedModel.maybeCameraOf(context);

  /// The camera of the closest [FlutterMap] ancestor. If this is called from a
  /// context with no [FlutterMap] ancestor a [StateError] will be thrown.
  static MapCamera of(BuildContext context) =>
      maybeOf(context) ?? (throw StateError('`MapCamera.of()` should not be called outside a `FlutterMap` and its descendants'));
  ...
  MapCamera withOptions(MapOptions options) {
    if (options.crs == crs && options.minZoom == minZoom && options.maxZoom == maxZoom) {
      return this;
    }
  ...
  static Point<double> calculateRotatedSize(
    ...
    final height = (nonRotatedSize.y * cosAngle) + (nonRotatedSize.x * sinAngle);
  ...
  Point<double> project(LatLng latlng, [double? zoom]) => crs.latLngToPoint(latlng, zoom ?? this.zoom);

  /// Calculates the [LatLng] for the given [point] using this camera's
  /// [crs] and [zoom] (or the provided [zoom]).
  LatLng unproject(Point point, [double? zoom]) => crs.pointToLatLng(point, zoom ?? this.zoom);

  LatLng layerPointToLatLng(Point point) => unproject(point);

  /// Calculates the scale for a zoom from [fromZoom] to [toZoom] using this
  /// camera\s [crs].
  double getZoomScale(double toZoom, double fromZoom) => crs.scale(toZoom) / crs.scale(fromZoom);
  ...
  Bounds? getPixelWorldBounds(double? zoom) => crs.getProjectedBounds(zoom ?? this.zoom);

  /// Calculates the [Offset] from the [pos] to this camera's [pixelOrigin].
  Offset getOffsetFromOrigin(LatLng pos) => project(pos).subtract(pixelOrigin).toOffset();
  ...
  Bounds<double> get pixelBounds => _pixelBounds ?? (_pixelBounds = pixelBoundsAtZoom(zoom));
  ...
  Point<double> latLngToScreenPoint(LatLng latLng) {
    final nonRotatedPixelOrigin = (project(center, zoom) - nonRotatedSize / 2.0).round();
  ...
  LatLng offsetToCrs(Offset offset, [double? zoom]) {
    ...
    final point = (offset.toPoint() - (nonRotatedSize / 2.0)).rotate(rotationRad);
