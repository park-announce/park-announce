...
class TileImageManager {
  ...
  bool containsTileAt(TileCoordinates coordinates) => _tiles.containsKey(coordinates.key);
  ...
  List<TileImage> inRenderOrder(double maxZoom, int currentZoom) {
    final result = _tiles.values.toList()..sort((a, b) => a.zIndex(maxZoom, currentZoom).compareTo(b.zIndex(maxZoom, currentZoom)));
  ...
  void markAsNoLongerCurrentOutside(int currentTileZoom, DiscreteTileRange noPruneRange) {
    for (final entry in _tiles.entries) {
      final tile = entry.value;
      final c = tile.coordinates;

      if (tile.current && (c.z != currentTileZoom || !noPruneRange.contains(Point(c.x, c.y)))) {
        tile.current = false;
      }
    }
  }
  ...
  void evictErrorTiles(
    ...
    if (evictStrategy == EvictErrorTileStrategy.notVisibleRespectMargin) {
      ...
      for (final entry in _tiles.entries) {
        ...
        if (tile.loadError && (!tile.current || !tileRange.contains(Point(c.x, c.y)))) {
          toRemove.add(entry.key);
        }
      }
