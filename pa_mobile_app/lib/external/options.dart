...
import 'package:pa_mobile_app/external/camera_constraint.dart';
...
class MapOptions {
  ...
  static MapOptions? maybeOf(BuildContext context) => FlutterMapInheritedModel.maybeOptionsOf(context);

  /// The options of the closest [FlutterMap] ancestor. If this is called from a
  /// context with no [FlutterMap] ancestor a [StateError] will be thrown.
  static MapOptions of(BuildContext context) =>
      maybeOf(context) ?? (throw StateError('`MapOptions.of()` should not be called outside a `FlutterMap` and its descendants'));
  ...
  InteractionOptions get interactionOptions =>
      ...
      InteractionOptions(
        ...
        pinchZoomWinGestures: _pinchZoomWinGestures ?? MultiFingerGesture.pinchZoom | MultiFingerGesture.pinchMove,
        pinchMoveThreshold: _pinchMoveThreshold ?? 40.0,
        pinchMoveWinGestures: _pinchMoveWinGestures ?? MultiFingerGesture.pinchZoom | MultiFingerGesture.pinchMove,
  ...
  CameraConstraint get cameraConstraint =>
      _cameraConstraint ?? (maxBounds != null ? CameraConstraint.contain(bounds: maxBounds!) : const CameraConstraint.unconstrained());
