...
class FitBounds extends CameraFit {
  ...
  MapCamera fit(MapCamera camera) {
    ...
    if (camera.rotation != 0.0) {
      ...
      final centerRotated = (swPointRotated + nePointRotated) / 2 + paddingOffset;
...
class FitInsideBounds extends CameraFit {
  ...
  static double _rectInRotRectScale({
    ...
    if (quadrant.isOdd) {
      final px = bigRectHalfWidth * math.cos(angleRad) + bigRectHalfHeight * math.sin(angleRad);
      final py = bigRectHalfWidth * math.sin(angleRad) - bigRectHalfHeight * math.cos(angleRad);
      final dx = -math.cos(angleRad);
      final dy = -math.sin(angleRad);

      if (smallRectHalfWidth * dy - smallRectHalfHeight * dx != 0) {
        var k = (px * dy - py * dx) / (smallRectHalfWidth * dy - smallRectHalfHeight * dx);
        if (quadrant >= 2) {
          k = -k;
        }
      ...
      if (-smallRectHalfWidth * dx + smallRectHalfHeight * dy != 0) {
        var k = (px * dx + py * dy) / (-smallRectHalfWidth * dx + smallRectHalfHeight * dy);
        if (quadrant >= 2) {
          k = -k;
        }
...
class FitCoordinates extends CameraFit {
  ...
  MapCamera fit(MapCamera camera) {
    ...
    final projectedPoints = [for (final coord in coordinates) camera.project(coord, newZoom)];

    final rotatedPoints = projectedPoints.map((point) => point.rotate(-camera.rotationRad));
  ...
  double _getCoordinatesZoom(
    ...
    final projectedPoints = [for (final coord in coordinates) camera.project(coord)];
